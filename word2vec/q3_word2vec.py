...
def normalizeRows(x):
    ...
    ### YOUR CODE HERE
    den = np.sqrt(np.sum(x*x,1,keepdims=1))
    x = x/den
    ### END YOUR CODE
...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    ### YOUR CODE HERE
    u0vc =  np.dot(outputVectors,predicted)
    soft_u0vc = softmax(u0vc)
    cost = -np.log(soft_u0vc[target])
    yhat_y = soft_u0vc
    yhat_y[target] -= 1
    gradPred = np.dot(outputVectors.T,yhat_y)
    grad = np.outer(yhat_y, predicted)
    ### END YOUR CODE
